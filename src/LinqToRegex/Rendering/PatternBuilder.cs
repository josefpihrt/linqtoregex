...
internal sealed class PatternBuilder
{
    ...
    private readonly LineInfoBuilder? _lineInfo;

    internal PatternBuilder()
        : this(new PatternOptions())
    {
    }

    internal PatternBuilder(PatternOptions options)
        : this(options, RegexOptions.None)
    {
    }

    internal PatternBuilder(PatternOptions options, RegexOptions regexOptions)
    {
        Options = options ?? throw new ArgumentNullException(nameof(options));

        _sb = new StringBuilder();

        if (options.Indented && (options.IncludeComment || options.IncludeInlineOptions))
            _lineInfo = new LineInfoBuilder();

        CurrentOptions = GetInlineOptions(regexOptions);

        static RegexOptions GetInlineOptions(RegexOptions options)
        {
            return options
                & ~(
#if NETFRAMEWORK
    ...
    {
        return (Options.CSharpLiteral || Options.VisualBasicLiteral)
            ? GetLiteral()
            : GetPattern();
    }
    ...
    {
        return (_lineInfo is not null)
            ? _lineInfo.AddComments(_sb.ToString(), Options)
            : _sb.ToString();
    }
        ...
        for (int i = 0; i < value!.Length; i++)
        {
            ...
            if (mode != CharEscapeMode.None)
            {
                ...
                if (i > 0)
                {
                    ...
                    if (_lineInfo is not null && !inCharGroup)
                        _lineInfo.AddInfo(value, i);
                }
                ...
                do
                {
                    ...
                    if (_lineInfo is not null && !inCharGroup)
                        _lineInfo.AddInfo(SyntaxKind.Character, ch);
                    ...
                    if ((i - lastPos) > 0)
                    {
                        ...
                        if (_lineInfo is not null && !inCharGroup)
                            _lineInfo.AddInfo(value, lastPos, i - lastPos);
                    }
                }
                while (i < value.Length);
    ...
    internal void AppendInternal(int value, bool inCharGroup)
    {
        ...
        if (_lineInfo is not null && !inCharGroup)
            _lineInfo.AddInfo(SyntaxKind.Character, (char)value);
    }
    ...
    public void AppendIfAssert(object testContent, object trueContent, object? falseContent)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.IfAssert);

        if (!Options.IfConditionWithoutAssertion)
            AppendDirect("?=");
    ...
    internal void AppendIfGroupInternal(string groupName, object trueContent, object? falseContent, bool checkGroupName)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.IfGroup);
    ...
    public void AppendAssertion(object content)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.Assertion);
    ...
    public void AppendNegativeAssertion(object content)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.NegativeAssertion);
    ...
    public void AppendBackAssertion(object content)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.BackAssertion);
    ...
    public void AppendNegativeBackAssertion(object content)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.NegativeBackAssertion);
    ...
    public void AppendBeginningOfInput()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.BeginningOfInput);
    }
    ...
    public void AppendBeginningOfInputOrLine()
    {
        ...
        _lineInfo?.AddInfo((IsCurrentOptions(RegexOptions.Multiline)) ? SyntaxKind.BeginningOfInputOrLine : SyntaxKind.BeginningOfInput);
    }
    ...
    public void AppendEndOfInput()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.EndOfInput);
    }
    ...
    public void AppendEndOfInputOrLine()
    {
        ...
        _lineInfo?.AddInfo((IsCurrentOptions(RegexOptions.Multiline)) ? SyntaxKind.EndOfInputOrLine : SyntaxKind.EndOfInput);
    }
    ...
    public void AppendEndOfInputOrBeforeEndingLinefeed()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.EndOfInputOrBeforeEndingLinefeed);
    }
    ...
    public void AppendWordBoundary()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.WordBoundary);
    }
    ...
    public void AppendNegativeWordBoundary()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.NegativeWordBoundary);
    }
    ...
    public void AppendPreviousMatchEnd()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.PreviousMatchEnd);
    }
    ...
    private void AppendNumberedGroupStart()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.Group);
    }
    ...
    {
        AppendGroupStart();
        AppendDirect(Options.OpenIdentifierBoundaryChar);
        AppendDirect(groupName);
        AppendDirect(Options.CloseIdentifierBoundaryChar);

        _lineInfo?.AddInfo(SyntaxKind.NamedGroup);
    ...
    internal void AppendNoncapturingGroupStart()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.NoncapturingGroup);
    }
    ...
    public void AppendNonbacktrackingGroup(object content)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.NonbacktrackingGroup);
    ...
    internal void AppendBalancingGroup(string name1, string name2, object content)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.BalancingGroup);
    ...
    internal void AppendGroupEnd(bool unindent)
    {
        ...
        if (_lineInfo is not null && unindent)
            _lineInfo.AddInfo(SyntaxKind.GroupEnd);
    }
    ...
    public void AppendAnyChar()
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.AnyChar);
    ...
    public void AppendAnyCharNative()
    {
        ...
        _lineInfo?.AddInfo((IsCurrentOptions(RegexOptions.Singleline)) ? SyntaxKind.AnyChar : SyntaxKind.AnyCharExceptLinefeed);
    }
    ...
    public void AppendDigit()
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo(SyntaxKind.Digit);
    }
    ...
    public void AppendNotDigit()
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo(SyntaxKind.NotDigit);
    }
    ...
    public void AppendWhiteSpace()
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo(SyntaxKind.WhiteSpace);
    }
    ...
    public void AppendNotWhiteSpace()
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo(SyntaxKind.NotWhiteSpace);
    }
    ...
    public void AppendWordChar()
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo(SyntaxKind.WordChar);
    }
    ...
    public void AppendNotWordChar()
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo(SyntaxKind.NotWordChar);
    }
    ...
    private void AppendCharGroupStart(bool negative, int charNumber)
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 1)
            _lineInfo.AddInfo((negative) ? SyntaxKind.NegativeCharGroup : SyntaxKind.CharGroup, (char)charNumber);
    }
    ...
    internal void AppendGeneralCategory(GeneralCategory category, bool negative)
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo(category, negative);
    }
    ...
    internal void AppendNamedBlock(NamedBlock block, bool negative)
    {
        ...
        if (_lineInfo is not null && _charGroupLevel == 0)
            _lineInfo.AddInfo((negative) ? SyntaxKind.NotNamedBlock : SyntaxKind.NamedBlock);
    }
    ...
    public void AppendMaybe(bool lazy)
    {
        ...
        if (_lineInfo is not null)
            _lineInfo.LastLine.QuantifierKind = QuantifierKind.Maybe;
    ...
    public void AppendMaybeMany(bool lazy)
    {
        ...
        if (_lineInfo is not null)
            _lineInfo.LastLine.QuantifierKind = QuantifierKind.MaybeMany;
    ...
    public void AppendOneMany(bool lazy)
    {
        ...
        if (_lineInfo is not null)
            _lineInfo.LastLine.QuantifierKind = QuantifierKind.OneMany;
    ...
    internal void AppendCountInternal(int exactCount)
    {
        ...
        if (_lineInfo is not null)
        {
            _lineInfo.LastLine.QuantifierKind = QuantifierKind.Count;
            _lineInfo.LastLine.Count1 = exactCount;
        }
    }
    ...
    internal void AppendCountInternal(int minCount, int maxCount)
    {
        ...
        if (_lineInfo is not null)
        {
            _lineInfo.LastLine.QuantifierKind = QuantifierKind.CountRange;
            _lineInfo.LastLine.Count1 = minCount;
            _lineInfo.LastLine.Count2 = maxCount;
        }
    }
    ...
    internal void AppendCountFromInternal(int minCount)
    {
        ...
        if (_lineInfo is not null)
        {
            _lineInfo.LastLine.QuantifierKind = QuantifierKind.CountFrom;
            _lineInfo.LastLine.Count1 = minCount;
        }
    }
    ...
    public void AppendMaybeCount(int maxCount, bool lazy)
    {
        ...
        if (_lineInfo is not null)
        {
            _lineInfo.LastLine.QuantifierKind = QuantifierKind.MaybeCount;
            _lineInfo.LastLine.Count1 = 0;
            _lineInfo.LastLine.Count2 = maxCount;
        }
    ...
    internal void AppendGroupReferenceInternal(int groupNumber)
    {
        ...
        if (!Options.Indented && Options.SeparateGroupNumberReference)
            AppendDirect("(?:)");

        _lineInfo?.AddInfo(SyntaxKind.GroupReference);
    }
    ...
    public void AppendOptions(RegexOptions applyOptions, RegexOptions disableOptions)
    {
        ...
        _lineInfo?.AddInfo(SyntaxKind.Options);
    ...
    internal void AppendOptions(RegexOptions applyOptions, RegexOptions disableOptions, object content)
    {
        ...
        if (applyOptions != RegexOptions.None || disableOptions != RegexOptions.None)
        {
            ...
            _lineInfo?.AddInfo(SyntaxKind.GroupOptions);
        }
        else
        {
            AppendNoncapturingGroupStart();
    ...
    {
        if ((Options.CSharpLiteral || Options.VisualBasicLiteral) && value == '"')
            _sb.Append('"');
    ...
    {
        if (Options.CSharpLiteral || Options.VisualBasicLiteral)
        {
            for (int i = 0; i < value.Length; i++)
            {
                if (value[i] == '"')
        ...
        if (Options.Indented)
        {
            if (_charGroupLevel == 0)
            {
                if (_pendingOr)
                {
                    ...
                    _lineInfo?.AddInfo(SyntaxKind.Or);
    ...
    {
        AppendLine();
        _sb.Append(' ', _indentLevel * Options.IndentSize);
    }
    ...
    /// <summary>
    /// Gets the <see cref="PatternOptions"/> object that modifies the pattern.
    /// </summary>
    public PatternOptions Options { get; }

    internal int Length => _sb.Length;

    internal Stack<CharGrouping> CharGroupings => _charGroupings ??= new Stack<CharGrouping>();
    ...
    private RegexOptions CurrentOptions
    {
        ...
        set
        {
            if (Options.Indented || Options.IncludeInlineOptions)
                _lineInfo!.CurrentOptions = value;
